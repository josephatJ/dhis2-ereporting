...
class _MetadataDownloadState extends State<MetadataDownload> {
  ...
  downloadIbsMetaData() async {
    ...
    setState(() {
      currentProcess = "Syncing programs";
      succesfullProcesses.add("Organisation units synced");
    });

    try {
      var programs = await D2Touch.programModule.program.download((p0, p1) {
        setState(() {
          processPercent = p0.percentage / 600;
          currentSubProcess = p0.message;
        });
    ...
    try {
      var programStagesSyncResponse = await ProgramQuery().download((p0, p1) {
        setState(() {
  ...
  downloadMcbsMetaData() async {
    ...
    try {
      var programSyncResponse = await ProgramQuery().download((p0, p1) {
        setState(() {
